...
public class XConomy extends JavaPlugin {
    ...
    public void logger(String tag, String message) {
        if (tag == null) {
            ...
            if (message == null) {
                getLogger().info(Messages.systemMessage(tag));
            } else {
                if (message.startsWith("<#>")) {
                    getLogger().info(message.substring(2) + Messages.systemMessage(tag));
                } else {
                    getLogger().info(Messages.systemMessage(tag) + message);
                }
            }
        }
    }
