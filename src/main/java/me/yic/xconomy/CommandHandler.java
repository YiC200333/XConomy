            ...
            case "eco": {
                ...
                switch (args.length) {
                    ...
                    case 5: {
                        ...
                        if (XConomy.issemionlinemode() && args[2].equalsIgnoreCase("online")) {
                            sender.sendMessage(sendMessage("prefix") + Messages.systemMessage("§c该指令不支持在半正版模式中使用"));
                            return true;
                        }

                        if (!check()) {
                            sender.sendMessage(sendMessage("prefix") + Messages.systemMessage("§cBungeeCord模式开启的情况下,无法在无人的服务器中使用OP命令"));
                            return true;
                        }
